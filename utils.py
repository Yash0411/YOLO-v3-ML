...
def generate_colors(class_names):
    ...
    colors = list(map(lambda x: ((x[0] * 255)/1000, (x[1] * 255)/1000, (x[2] * 255)/1000), colors))
...
# draw all results
def draw_boxes(filename, v_boxes, v_labels, v_scores, labels, colors):
    # load the image
    ...
    for i in range(len(v_boxes)):
        ...
        pyplot.text(x1, y1, label, color=colors[i])
    # show the plot
    pyplot.show()